...
def train(attn_implementation=None):
    ...
    # target_modules = find_all_linear_names(model)
    # with open("/remote-home/share/22-changkaiji-22210860023/onlyffn-model_parameters.txt", "w", encoding="utf-8") as f:
    #     f.write(f"Total parameters: ~{total_params/1e6:.2f} MB)")
    #     f.write(f"Trainable parameters: ~{trainable_params/1e6:.2f} MB)")
    #     for name, param in model.named_parameters():
    #         trainable = "Yes" if param.requires_grad else "No"
    #         f.write(f"{name}, Requires Grad: {trainable}, Shape: {param.shape}\n")
    #     f.write("Target modules:\n")
    ...
    try:
        new_dir = os.path.join(training_args.output_dir, "new")
        safe_save_model_for_hf_trainer(trainer=trainer, output_dir=new_dir)
        rank0_print(f"Model saved to {training_args.output_dir}")
    except Exception as e:
        print('1')
        print(e)
    try:
        torch.save(model.state_dict(), os.path.join(training_args.output_dir, "all_trainables_new.bin"))
    except Exception as e:
        print('2')
        print(e)


    # output_file = f'{training_args.output_dir}/pico_test_rag_llava_6144_gpt_format_with_query-moe.jsonl'
    # image_processor = vision_tower.image_processor
    # batch_size = 10
    # data = read_json('/remote-home/share/22-changkaiji-22210860023/evidence_synthesis/final/pico_test_rag_llava_6144_gpt_format_with_query.json')
    # batch_results = []
    # for idx, item in enumerate(tqdm(data)):
    #     response, answer = eval_single_data(item, model, tokenizer, image_processor)       
