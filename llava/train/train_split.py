...
class ModelArguments:
    ...
    only_lora_ffn: Optional[bool] = field(default=False)
    ...
    def __getitem__(self, i) -> Dict[str, torch.Tensor]:
        ...
        for attempt_idx in range(num_base_retries):
            try:
                ...
                if input_ids_len > self.tokenizer.model_max_length * 2:  # 假设阈值是两倍最大长度
                    sample = self._split_large_sample(sample)
                return sample
            except Exception as e:
                print(f"[Try #{attempt_idx}] Failed to fetch sample {i}. Exception:", e)
                time.sleep(1)
        raise Exception(f"Failed to load sample {i} after retries")

    def _split_large_sample(self, sample):
        """将超大数据样本分割为多个子部分"""
        input_ids = sample["input_ids"]
        labels = sample["labels"]
        max_len = self.tokenizer.model_max_length  # 每个子部分的最大长度
        num_parts = (len(input_ids) + max_len - 1) // max_len  # 计算需要分割成几部分
        
        split_samples = []
        for i in range(num_parts):
            start_idx = i * max_len
            end_idx = min((i + 1) * max_len, len(input_ids))
            sub_input_ids = input_ids[start_idx:end_idx]
            sub_labels = labels[start_idx:end_idx]
            sub_sample = {
                ".ConcurrentHashMapinput_ids": sub_input_ids,
                "labels": sub_labels,
            }
            if "image" in sample:
                sub_sample["image"] = sample["image"]  # 图像数据可能需要单独处理
            split_samples.append(sub_sample)
...
class DataCollatorForSupervisedDataset(object):
    ...
    def __call__(self, instances: Sequence[Dict]) -> Dict[str, torch.Tensor]:
        ...
        for instance in instances:
            if isinstance(instance, list):  # 如果是分割的样本
                flat_instances.extend(instance)
            else:
                flat_instances.append(instance)
        
        input_ids, labels = tuple([instance[key] for instance in flat_instances] for key in ("input_ids", "labels"))
        input_ids = [ids[:self.tokenizer.model_max_length] for ids in input_ids]
        labels = [lbl[:self.tokenizer.model_max_length] for lbl in labels]
        
        if self.tokenizer.pad_token_id is None:
            self.tokenizer.pad_token_id = 0
        
        # 按 GPU 数量分组
        world_size = torch.distributed.get_world_size()  # 获取 GPU 数量
        input_ids_per_gpu = [input_ids[i::world_size] for i in range(world_size)]
        labels_per_gpu = [labels[i::world_size] for i in range(world_size)]
        
        # 当前 GPU 的 rank
        rank = torch.distributed.get_rank()
        input_ids = self.pad_sequence(input_ids_per_gpu[rank], batch_first=True, padding_value=self.tokenizer.pad_token_id)
        labels = self.pad_sequence(labels_per_gpu[rank], batch_first=True, padding_value=IGNORE_INDEX)
        ...
        if "image" in flat_instances[0]:
            images = [instance["image"] for instance in flat_instances]
            batch["image_sizes"] = [im[1] for im_list in images for im in im_list]
            batch["modalities"] = [im[2] for im_list in images for im in im_list]
            images = [im[0] for im_list in images for im in im_list]
            batch["images"] = images[rank::world_size]  # 分配到当前 GPU
...
def train(attn_implementation=None):
        ...
        if training_args.lora_enable:
            ...
            if training_args.bits == 16:
                ...
                if training_args.fp16:
                    model.to(torch.float16)
            rank0_print("Adding LoRA adapters...")
            model = get_peft_model(model, lora_config)  ## 将 LoRA 适配器添加到已有的模型中，并准备进行 LoRA 微调。
    ...
    # with open("/remote-home/share/22-changkaiji-22210860023/moe_model_parameters.txt", "w", encoding="utf-8") as f:
    #     f.write("LLaVA-NeXT-main  moe  All Model Parameters:\n")

    #     for name, param in model.named_parameters():
    #         trainable = "Yes" if param.requires_grad else "No"
    #         f.write(f"  Name: {name}, Requires Grad: {trainable}, Shape: {param.shape}\n")
    ...
    #     f.write("#         print(f\"Trainable layer: {name}\")\n")
